...
function productImage(p) {
  return PRODUCT_PHOTOS[p.image] || 'assets/' + ({Componentes:'cpu',Periféricos:'keyboard',Consolas:'console'})[p.category] + '.svg';
}
function describeImage(p) {
  return (PRODUCT_PHOTOS[p.image] ? 'Fotografía referencial de ' : 'Ilustración de categoría para ') + p.name;
}
...
const activeSession=session();if(activeSession){$('account-link').textContent=activeSession.role==='admin'?'Administrador':'Mi cuenta';$('logout').hidden=false;$('admin-link').hidden=!admin();}
...
function editProduct(p){if(!ensureAdmin())return;const form=$('product-form');form.reset();form.querySelectorAll('.error').forEach(e=>e.textContent='');form.querySelectorAll('[aria-invalid]').forEach(e=>e.removeAttribute('aria-invalid'));$('product-id').value=p?.id||'';$('product-form-title').textContent=p?'Editar producto':'Nuevo producto';if(p){$('code').value=p.code;$('product-name').value=p.name;$('product-category').value=p.category;$('price').value=p.price;$('stock').value=p.stock;$('active').checked=p.active;}form.hidden=false;$('code').focus();}
if($('admin-panel')){if(!admin()){$('admin-notice').textContent='Necesitas una cuenta de administrador para gestionar el inventario. ';const a=element('a','Iniciar sesión');a.href='login.html';$('admin-notice').append(a);}else{$('admin-notice').textContent='Sesión de administrador de demostración. Los cambios se guardan en este navegador.';$('admin-panel').hidden=false;drawInventory();$('new-product').addEventListener('click',()=>editProduct(null));$('cancel-product').addEventListener('click',()=>{$('product-form').hidden=true;});const integerRule=(min,max,label)=>el=>el.value.trim()===''||!Number.isInteger(Number(el.value))||Number(el.value)<min||Number(el.value)>max?label+' debe ser un entero entre '+min+' y '+max+'.':'';const rules={code:el=>! /^[A-Za-z0-9-]{3,20}$/.test(el.value.trim())?'Usa entre 3 y 20 letras, números o guiones.':products.some(p=>p.code.toLowerCase()===el.value.trim().toLowerCase()&&String(p.id)!==$('product-id').value)?'Este código ya existe. Usa uno diferente.':'','product-name':lengthRule(3,80,'El nombre'),price:integerRule(1,99999999,'El precio'),stock:integerRule(0,9999,'El stock')};bindValidation(rules);$('product-form').addEventListener('submit',event=>{event.preventDefault();if(!ensureAdmin()||!validate(rules))return;const id=Number($('product-id').value),old=products.find(p=>p.id===id);const product={id:id||Math.max(0,...products.map(p=>p.id))+1,code:$('code').value.trim().toUpperCase(),name:$('product-name').value.trim(),category:$('product-category').value,price:Number($('price').value),stock:Number($('stock').value),active:$('active').checked,image:old?.image||'',description:old?.description||'Producto del catálogo NEXUS'};if(old)products=products.map(p=>p.id===id?product:p);else products.push(product);save('nexus_products',products);reconcileCart();storeCart();drawInventory();$('product-form').hidden=true;notify(old?'Producto actualizado.':'Producto creado.');});}}
